...
jQuery(document).ready(function () {
  ...
  if (jQuery.cookie) {
    if (!jQuery.isEmptyObject(jQuery.cookie())) {
      deleteAllCookiesHTML =
        '<span class="fa fa-trash-o" data-toggle="tooltip" data-placement="top" title="Clear All Styles (Page will be reloaded)"></span>';
    }
  }
  ...
  jQuery('#boxed_margin')
    .find('input')
    .on('change', switcherClass.switchBoxedMargins);

  //pattern switcher
  jQuery('#switcher-patterns a').on(
    'click',
    { patterns: patternClasses },
    function (e) {
  ...
  if (cookieClass.returnTrueOrFalseCookie('colorScheme')) {
    jQuery(
      '#switcher-colors a[data-color="' +
        cookieClass.getCookieValue('colorScheme') +
        '"]'
    ).trigger('click');
  }
  ...
  if (cookieClass.returnTrueOrFalseCookie('boxed')) {
    if (cookieClass.returnTrueOrFalseCookie('topBottomMargins')) {
      var topBottomMarginsCookie =
        cookieClass.getCookieValue('topBottomMargins');
    }
    ...
    if (patternCookie) {
      jQuery('#switcher-patterns')
        .find('a[data-pattern="' + patternCookie + '"]')
        .trigger('click');
    }
  }
});
